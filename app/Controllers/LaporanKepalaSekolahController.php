...
class LaporanKepalaSekolahController extends BaseController
{
    protected $db;
    ...
    public function index()
    {
        ...
        $kelasOptions   = $this->getKelasOptions($filters['tahun_ajaran_id']);
        ...
        return view('kepsek/laporan/index', [
            ...
            'selectedKelas'       => $this->getKelasById($filters['kelas_id']),
            'menu'                => 'laporan',
        ]);
    }
    ...
    public function export()
    {
        ...
        $kelas          = $this->getKelasById($filters['kelas_id']);
        $viewMode       = empty($filters['periode']) ? 'rekap' : 'detail';
        ...
        $kelasLabel = !empty($kelas['nama_kelas']) ? $kelas['nama_kelas'] : 'semua_kelas';
        $namaTA     = $tahunAjaran['nama_tahun_ajaran'] ?? 'semua_tahun_ajaran';
        $prefix     = $viewMode === 'rekap' ? 'rekap_spp_kepala_sekolah_' : 'detail_spp_kepala_sekolah_';

        $filename = $prefix
            . str_replace(['/', ' '], ['-', '_'], $namaTA) . '_'
            . str_replace([' ', '/'], ['_', '-'], $kelasLabel) . '_'
            . str_replace([' ', '/'], ['_', '-'], $periodeLabel) . '.xls';

        $html = view('kepsek/laporan/export_excel', [
            'rows'                => $rows,
            'filters'             => $filters,
            'summary'             => $summary,
            'viewMode'            => $viewMode,
    ...
    private function getLaporanDetailData(array $filters): array
    {
        ...
        if (!empty($filters['kelas_id'])) {
            $builder->where('k.id', (int) $filters['kelas_id']);
        }
    ...
    private function getMatchedStudentIdsForStatusFilter(array $filters, array $studentIds): ?array
    {
        ...
        $builder->join('kelas_tahun_ajaran kta', 'kta.id = t.kelas_tahun_ajaran_id');
        $builder->join('kelas k', 'k.id = kta.kelas_id');
    ...
    private function getKelasById($kelasId): ?array
    {
        if (empty($kelasId)) {
            return null;
        }

        $row = $this->db->table('kelas')

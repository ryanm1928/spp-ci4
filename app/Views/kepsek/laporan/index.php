<?= $this->extend('kepsek/template/layout') ?>
...
function badgeStatusKepsek($status)
{
    if ($status === 'lunas') {
...
function statusMarkKepsek($status)
{
    if ($status === 'lunas') {
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1 text-gray-800"><i class="fas fa-wallet text-primary mr-2"></i>Laporan SPP Kepala Sekolah</h1>
            <p class="mb-0 text-muted">Laporan global seluruh kelas dengan filter per kelas.</p>
        </div>
        <a href="<?= site_url('kepala-sekolah/laporan/export?' . $queryExport) ?>" class="btn btn-success shadow-sm">
    ...
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-body bg-light rounded">
            <form method="get" action="<?= site_url('kepala-sekolah/laporan') ?>" id="filterForm">
                <div class="row align-items-end">
                    ...
                    <div class="col-md-3 mb-3 mb-md-0">
                        <label class="form-label font-weight-bold"><i class="fas fa-school mr-1"></i> Kelas</label>
                    ...
                    <div class="col-md-2">
                        <div class="d-flex">
                            ...
                            <a href="<?= site_url('kepala-sekolah/laporan') ?>" class="btn btn-light border shadow-sm " style="margin-left: 10px; title="Reset Filter">
            ...
            <div class="card-header bg-white py-3 d-flex flex-row align-items-center justify-content-between">
                <div>
                    ...
                    <?php if (!empty($selectedKelas)): ?>
                        <span class="badge badge-info px-3 py-2 ml-2">Kelas: <?= esc($selectedKelas['nama_kelas']) ?></span>
                    <?php else: ?>
                        <span class="badge badge-secondary px-3 py-2 ml-2">Semua Kelas</span>
                    <?php endif; ?>
                </div>
                                ...
                                <?php foreach ($rows as $i => $row): ?>
                                    <tr>
                                        ...
                                        <?php foreach ($periodeOptions as $periodeKey => $label): ?>
                                            ...
                                            <td class="text-center align-middle"><?= statusMarkKepsek($cell['status']) ?></td>
                                        <?php endforeach; ?>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
        ...
        <div class="card shadow-sm border-0 rounded-lg">
            <div class="card-header bg-white py-3 d-flex flex-row align-items-center justify-content-between border-bottom-0">
                <div>
                    ...
                    <?php if (!empty($selectedTahunAjaran)): ?>
                        <span class="badge badge-primary px-3 py-2 ml-2 shadow-sm rounded-pill">TA: <?= esc($selectedTahunAjaran['nama_tahun_ajaran']) ?></span>
                    <?php endif; ?>
                    <?php if (!empty($selectedKelas)): ?>
                        <span class="badge badge-info px-3 py-2 ml-2 shadow-sm rounded-pill">Kelas: <?= esc($selectedKelas['nama_kelas']) ?></span>
                    <?php else: ?>
                        <span class="badge badge-secondary px-3 py-2 ml-2 shadow-sm rounded-pill">Semua Kelas</span>
                    <?php endif; ?>
                </div>
                <div class="btn-group btn-group-sm shadow-sm" role="group">
                    <button type="button" class="btn btn-outline-primary" onclick="zoomTable(-0.1)" title="Zoom Out"><i class="fas fa-search-minus"></i></button>
                    ...
                    <table class="table table-bordered table-hover align-middle mb-0 table-dynamic zoomable-table">
                        <thead class="bg-light">
                            <tr class="text-uppercase text-dark text-xs font-weight-bold">
                                ...
                                <th class="py-3 text-center col-fit">Periode</th>
                                <th class="py-3 col-expand">Identitas Siswa</th>
                                <th class="py-3 text-right">Tagihan</th>
                                <th class="py-3 text-right">Terbayar</th>
                                <th class="py-3 text-right">Sisa</th>
                                <th class="py-3 text-center col-fit">Jatuh Tempo</th>
                                <th class="py-3 text-center col-fit">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (!empty($rows)): ?>
                                <?php foreach ($rows as $i => $row): ?>
                                    ...
                                    <tr class="<?= $rowClass ?>">
                                        ...
                                        <td class="align-middle text-center py-3">
                                            <?= badgeStatusKepsek($row['status_pembayaran']) ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
